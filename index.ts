...
let maxTemp: number = Math.max(21,23,33,32,16);
console.log(maxTemp);
...
function removeFalseValue(array: any[]): any[] {
...
// Question 27
// Write a function that receives an array of names and formats them into a
// string separated by commas, except for the last two names, which should be separated by "and".
// Assuming each name is an object with 'first' and 'last' properties
type NameProps = {
    first: string;
    last: string;
};
function formatNames(namesList: NameProps[]): string {
    const formattedNames: string[] = namesList.map((item, index) => {
...
const fahrenheitTemperatures: number[] = [77, 68, 90, 97];
...
const Array7: number[] = [10, 20, 30, 40];
swapElements(Array7, 1, 3); 
console.log("Swapped array:", Array7);


// Question 31
function countOccurrences1(search: string, char: string): number {
...
// Question 32
// Create a 'to-do list' array of objects where each object has properties task
// and completed (a boolean). Write a function to log only the tasks that are not yet completed.
...
// Question 35
// Write a script that simulates a basic calculator. It should take two operands and an operator ('+', '-', '*', '/')
// from the user, perform the operation, and log the result.
